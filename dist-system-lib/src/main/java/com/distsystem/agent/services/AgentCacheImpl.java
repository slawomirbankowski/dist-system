...
/** service to connect all cache storages, policies, agents
 * to perform clean based on time
 * replace all cache with fresh objects
 *
 * */
public class AgentCacheImpl extends CacheBase {

    /** local logger for this class*/
    protected static final Logger log = LoggerFactory.getLogger(AgentCacheImpl.class);
    ...
    public AgentCacheImpl(Agent agent) {
        super(agent);
    ...
    protected boolean onReinitialize() {
        ...
        createEvent("CacheManager", AgentEvent.EVENT_CACHE_START);
        initializeStorages();
        initializeTimer();
        initializeEncoder();
    ...
    public long countObjectsService() {
        return 2L + storages.size()*2L + storages.values().stream().mapToLong(s -> s.getObjectsCount()).sum();
    }
    ...
    protected DistWebApiProcessor additionalWebApiProcessor() {
        return new DistWebApiProcessor(getServiceType())
                ...
                .addHandlerGet("storages-info", (m, req) -> req.responseOkJsonSerialize(getStoragesInfo()))
                .addHandlerGet("storage-keys", (m, req) -> req.responseOkJsonSerialize(getStorageKeys()))
                ...
                .addHandlerGet("items-per-storage", (m, req) -> req.responseOkJsonSerialize(getItemsCountPerStorage()))
                ...
                .addHandlerGet("object-keys", (m, req) -> req.responseOkJsonSerialize(getCacheKeys(req.getParamOne(), true)))
                .addHandlerGet("object-infos", (m, req) -> req.responseOkJsonSerialize(getCacheInfos(req.getParamOne(), true)))
    ...
    private void initializeStorages() {
        createEvent("initializeStorages", AgentEvent.EVENT_INITIALIZE_STORAGES);
    ...
    private boolean initializeSingleStorage(String className) {
        if (className==null || className.equals("null") || className.equals("")) {
            return false;
        }
        try {
            cacheStats.initializeSingleStorage();
            String fullClassName = "com.distsystem.storage." + className;
            createEvent("initializeStorages", AgentEvent.EVENT_INITIALIZE_STORAGE, fullClassName);
            ...
            if (prevStorage != null) {
                ...
                createEvent("initializeStorages", AgentEvent.EVENT_DISPOSE_STORAGE, fullClassName);
                prevStorage.disposeStorage();
            }
            return true;
        } catch (Exception ex) {
            addIssueToAgent("initializeSingleStorage", ex);
    ...
    protected void onClose() {
        createEvent("onClose", AgentEvent.EVENT_CLOSE_BEGIN);
        ...
        parentAgent.close();
        createEvent("onClose", AgentEvent.EVENT_CLOSE_END);
    }
    ...
    public AgentWebApiResponse setCacheObjectFromRequest(AgentWebApiRequest req) {
        ...
        return req.responseOkJsonSerialize(setBack.toInfo());
    }
    ...
    public AgentWebApiResponse putCacheObjectsFromRequest(AgentWebApiRequest req) {
        ...
        if (objs == null || objs.size() == 0) {
            ...
            return req.responseOkJsonSerialize(setBacks);
        }
    }
    ...
    public AgentWebApiResponse getCacheValuesFromRequest(AgentWebApiRequest req) {
        ...
        return req.responseOkJsonSerialize(objs);
    }
    ...
    public int clearCaches(CacheClearMode clearMode) {
        createEvent("clearCaches", AgentEvent.EVENT_CACHE_CLEAN);
